...
begin transaction 
	----------------t1-------------
	select @@SPID, 'insert PROGRESS' 'result',* from PROGRESS where IDSTUDENT = 1003 ;
	select @@SPID, 'update PROGRESS' 'result',IDSTUDENT,NOTE from PROGRESS where IDSTUDENT = 1005 and NOTE = 6;
	commit;
	-----------t2-----------------
...
begin transaction 
	...
	update PROGRESS set NOTE = 6 where IDSTUDENT = 1005
	--------t1---------
	--------t2---------
	rollback;
...
update PROGRESS set IDSTUDENT  =1001 WHERE IDSTUDENT = 2107;
commit  -- внутренняя 
if @@TRANCOUNT > 0 -- внешняя
rollback;
select
	(select count(*) from progress where IDSTUDENT = 1001) 'E. of 1001',
